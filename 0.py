...
def save_alert_to_json(symbol):
    ...
    if symbol not in data:
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {symbol} alerts.json dosyasına kaydedildi."
        )
    else:
        print(
...
def git_push():
    try:
        ...
        if status:
            ...
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Değişiklikler başarıyla push edildi"
            )
        else:
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Değişiklik yok, push işlemi atlandı"
            )
    except subprocess.CalledProcessError as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Git hatası: {str(e)}"
        )
    except Exception as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Hata: {str(e)}")
...
def initialize_db(file_name):
    if not os.path.exists(file_name):
        ...
        conn.close()
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Veritabanı başlatıldı: {file_name}"
        )
...
def get_previous_volume(symbol):
    ...
    try:
        with sqlite3.connect("db/coin_alertsOLD.db") as conn:
            cursor = conn.cursor()
            cursor.execute(
                "SELECT previous_volume FROM coin_volumes WHERE symbol = ?",
                (symbol, ))
            result = cursor.fetchone()
            return result[0] if result else 0
    except sqlite3.Error as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] DB hatası: {str(e)}"
        )
        return 0
...
def save_current_volume(symbol, volume, db_file="db/coin_alertsNEW.db"):
    ...
    conn.close()
    print(
        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {symbol} anlık hacmi güncellendi: {volume}"
    )
...
def rotate_db():
    try:
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Anlık veritabanı döndürüldü"
        )
    except Exception as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Anlık DB döndürme hatası: {str(e)}"
        )
...
def get_previous_volume_1h(symbol):
    ...
    cursor = conn.cursor()
    cursor.execute("SELECT previous_volume FROM coin_volumes WHERE symbol = ?",
                   (symbol, ))
    result = cursor.fetchone()
    conn.close()
    return result[0] if result else 0
...
def rotate_db_1h():
    try:
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 1h veritabanı döndürüldü"
        )
    except Exception as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 1h DB döndürme hatası: {str(e)}"
        )
...
def rotate_db_24h():
    try:
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 24h veritabanı döndürüldü"
        )
    except Exception as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] 24h DB döndürme hatası: {str(e)}"
        )
...
def escape_markdown(text):
    ...
    return ''.join(
        ['\\' + char if char in escape_chars else char for char in str(text)])
...
def create_alert_table(title, headers, data):
    ...
    header_row = " | ".join(
        h.replace('(', '').replace(')', '') for h in headers)
    table_lines.append(header_row)
    table_lines.append("-" * (sum(len(h)
                                  for h in headers) + (len(headers) - 1) * 3))
    ...
    for row in data:
        ...
        for cell in row:
            if isinstance(cell, (int, float)):
                cell_str = f"{cell:,.2f}" if isinstance(cell,
                                                        float) else str(cell)
            else:
                cell_str = str(cell)
...
async def fetch_coingecko_data():
    print(
        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] CoinGecko verileri çekiliyor..."
    )
    try:
        params = {
            "vs_currency": "usd",
        ...
        with ThreadPoolExecutor() as executor:
            response = await asyncio.get_event_loop().run_in_executor(
                executor, requests.get, CONFIG["API_URLS"]["coingecko"],
                params)
        coins = response.json()
        ...
        for coin in coins:
            ...
            if market_cap and volume and market_cap > 0:
                ...
                if ratio >= CONFIG["ALERT_THRESHOLDS"]["volume_ratio"]:
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] CoinGecko Volume Alert: {symbol} (V/MCAP: {ratio:.1f}%)"
                    )
            ...
            if low_price and current_price and low_price > 0:
                ...
                if deviation <= CONFIG["ALERT_THRESHOLDS"][
                        "support_deviation"]:
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] CoinGecko Support Alert: {symbol} (Deviation: {deviation:.1f}%)"
                    )
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] CoinGecko taraması tamamlandı: {len(volume_alerts)} volume, {len(support_alerts)} support alert"
        )
        return volume_alerts, support_alerts
    except Exception as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] CoinGecko hatası: {str(e)}"
        )
        return [], []
...
async def fetch_binance_data():
    print(
        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Binance verileri çekiliyor..."
    )
    try:
        with ThreadPoolExecutor() as executor:
            response = await asyncio.get_event_loop().run_in_executor(
        ...
        for ticker in tickers:
            ...
            if previous_volume > 0:
                ...
                if inflow_change >= CONFIG["ALERT_THRESHOLDS"][
                        "inflow_threshold"] / 100:
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Binance Inflow Alert: {symbol} ({inflow_change:.2%})"
                    )
                ...
                if volume_change >= CONFIG["ALERT_THRESHOLDS"][
                        "volume_spike"] / 100:
                    print(
                        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Binance Volume Alert: {symbol} ({volume_change:.2%})"
                    )
            ...
            if price_change >= CONFIG["ALERT_THRESHOLDS"]["pump_threshold"]:
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Binance Pump Alert: {symbol} ({price_change:.1f}%)"
                )
                pump_alerts.append({
                    'symbol': symbol,
                    'price': last_price,
            ...
            if low_price > 0 and last_price <= low_price * (
                    1 + CONFIG["ALERT_THRESHOLDS"]["support_deviation"] / 100):
                deviation = ((last_price - low_price) / low_price) * 100
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Binance Support Alert: {symbol} (Deviation: {deviation:.1f}%)"
                )
                support_alerts.append({
                    'symbol': symbol,
                    'price': last_price,
            ...
            if previous_volume_24h > 0:
                vol_change_24h = (current_volume / previous_volume_24h -
                                  1) * 100
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Binance taraması tamamlandı: {len(pump_alerts)} pump, {len(support_alerts)} support, {len(inflow_alerts)} inflow, {len(volume_spike_alerts)} volume"
        )
        return pump_alerts, support_alerts, inflow_alerts, volume_spike_alerts, volume_increase_1h, volume_increase_24h
    except Exception as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Binance hatası: {str(e)}"
        )
        return [], [], [], [], [], []
    ...
    for chat_id in CONFIG["CHAT_IDS"]:
        ...
        while retries < max_retries:
            try:
                ...
                retries += 1
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {chat_id} için mesaj gönderme hatası (Deneme {retries}/{max_retries}): {str(e)}"
                )
...
async def main_loop():
    ...
    print(
        f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Sistem başlatıldı")
    while True:
        try:
            ...
            if cg_volume:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                    format_volume(item['volume']), f"{item['ratio']:.1f}%"
                ] for item in cg_volume]
                message_parts.append(
                    create_alert_table("COINGECKO VOLUME ALERTS (V/MCAP >20%)",
                                       ["Coin", "Price", "Volume", "Ratio"],
                                       table_data))
            # CoinGecko Support Alerts
            if cg_support:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                    f"${item['low']:.2f}", f"{item['deviation']:+.2f}%"
                ] for item in cg_support]
                message_parts.append(
                    create_alert_table(
                        "COINGECKO SUPPORT ZONE (Price ≤102% of 24h Low)",
                        ["Coin", "Price", "24h Low", "Deviation"], table_data))
            # Binance Pump Alerts
            if bn_pump:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                ...
                message_parts.append(
                    create_alert_table("BINANCE PUMP ALERTS (24h Change >20%)",
                                       ["Coin", "Price", "Change"],
                                       table_data))
            ...
            if bn_support:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                    f"${item['low']:.2f}", f"{item['deviation']:+.2f}%"
                ] for item in bn_support]
                message_parts.append(
                    create_alert_table(
                        "BINANCE SUPPORT ZONE (Price ≤102% of 24h Low)",
                        ["Coin", "Price", "24h Low", "Deviation"], table_data))
            # Binance Inflow Alerts
            if bn_inflow:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                    format_volume(item['volume']), f"{item['change']:.2%}"
                ] for item in bn_inflow]
                message_parts.append(
                    create_alert_table(
                        "BINANCE INFLOW ALERTS (1h Volume Increase >1%)",
                        ["Coin", "Price", "Volume", "Change"], table_data))
            # Binance Volume Spike Alerts
            if bn_volume:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                    format_volume(item['volume']), f"{item['change']:.2%}"
                ] for item in bn_volume]
                message_parts.append(
                    create_alert_table(
                        "BINANCE VOLUME SPIKE (1h Volume Increase >5%)",
                        ["Coin", "Price", "Volume", "Change"], table_data))
            # --- EKSTRA: 1h Volume Increase (>=2%) ---
            if bn_vol_incr_1h:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                    format_volume(item['volume']), f"{item['change']:.2f}%"
                ] for item in bn_vol_incr_1h]
                message_parts.append(
                    create_alert_table("BINANCE 1h VOLUME INCREASE (>=2%)",
                                       ["Coin", "Price", "Volume", "Change"],
                                       table_data))
            else:
                message_parts.append(
                    "*BINANCE 1h VOLUME INCREASE (>=2%)*\nNo coins found")
            # --- EKSTRA: 24h Volume Increase (>=50%) ---
            if bn_vol_incr_24h:
                table_data = [[
                    item['symbol'], f"${item['price']:.2f}",
                    format_volume(item['volume']), f"{item['change']:.2f}%"
                ] for item in bn_vol_incr_24h]
                message_parts.append(
                    create_alert_table("BINANCE 24h VOLUME INCREASE (>=50%)",
                                       ["Coin", "Price", "Volume", "Change"],
                                       table_data))
            else:
                message_parts.append(
                    "*BINANCE 24h VOLUME INCREASE (>=50%)*\nNo coins found")
            ...
            if len(message_parts) > 1:
                ...
                for part in parts:
                    ...
                    if not success:
                        print(
                            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Mesaj gönderilemedi"
                        )
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] {len(parts)} mesaj başarıyla gönderildi"
                )
            else:
                print(
                    f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Yeni alarm bulunamadı"
                )
            ...
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Tarama süresi: {duration:.2f}s"
            )
            await asyncio.sleep(CONFIG["SCAN_INTERVAL"])
        except Exception as e:
            print(
                f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Kritik hata: {str(e)}"
            )
            await asyncio.sleep(CONFIG["ERROR_RETRY_DELAY"])


# --- GLOBAL DEĞİŞKENLERİN BAŞLANGIÇ DEĞERLERİ ---
bn_pump = []
bn_support = []
cg_volume = []
current_data_lock = threading.Lock()  # Thread güvenliği için
...
@app.route('/status')
def status():
    """Sistem durumunu gösteren endpoint"""
    return jsonify({
        "status": "running",
...
@app.route('/alerts')
def get_alerts():
    """Son alarmları gösteren endpoint"""
    return jsonify({
        "binance_pump": bn_pump,
        "binance_support": bn_support,
        "coingecko_volume": cg_volume
    })
...
if __name__ == "__main__":
    try:
        ...
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Bot kapatılıyor..."
        )
    except Exception as e:
        print(
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Beklenmeyen hata: {str(e)}"
        )
